...
int nfc_sendcmd(usb_device_info *devinfo, unsigned char *buf, int size,unsigned char *res,int reslen) {
...
int packet_setcommandtype(usb_device_info *devinfo) {
  ...
  return nfc_sendcmd(devinfo, cmd, sizeof(cmd),NULL,0);
}
...
int packet_sens_req(usb_device_info *devinfo, unsigned char *buf,int size) {
  ...
  return nfc_sendcmd(devinfo, cmd, sizeof(cmd),buf,size);  
}
int packet_felicaread(usb_device_info *devinfo, unsigned char *buf,int size,unsigned char *idm) {
  ...
  //04 (InCommRF) 43 60 (timeout) 10 (size including size)
  //06 (Read W/O Enc) xx xx xx xx xx xx xx xx (idm)
  //01 (num of sev code) 0b 30 (sev code) 01 (num of blk code) 80 00 (blk code)
  return nfc_sendcmd(devinfo, cmd, sizeof(cmd), buf, size);
}
  ...
  while (1){      
    ...
    if(rbuf[9] == 0x05 && rbuf[10] == 0x00){
      ...
      if(rbuf[6] == 0x14 && rbuf[7] == 0x01){
        ...
        packet_felicaread(devinfo,rbuf,255,idm);

        // Check begining 1000 or 1100
        if (rbuf[28]=='1' && (rbuf[29]=='0' || rbuf[29]=='1') && rbuf[30]=='0' && rbuf[31]=='0'){
          if (rbuf[32]=='1') printf("student s");
          else if (rbuf[32]=='5') printf("student m");
          else if (rbuf[32]=='8') printf("student d");
          for (i=32;i<39;i++)
            printf("%c",rbuf[i]);
          printf("\n");
        } else {
